...
if class_balance_method == 'class_weight':
    param_space = {'penalty':hp.choice('penalty',['l1','l2']),'C':hp.loguniform('C',-7,5),'class_weight':hp.choice('class_weight',['balanced'])}
else:
    param_space = {'penalty':hp.choice('penalty',['l1','l2']),'C':hp.loguniform('C',-7,5)}
...
def run_trials():
    ...
    #Optimize
    best = fmin(fn=logistic_objective, space=param_space, algo=tpe.suggest, max_evals=max_trials, trials=trials, rstate = rand_state)
...
def get_results(trials, X_train, y_train):
    ...
    def compute_cv_metric(split, cross_val_data, bayes_trials_results):
        ...
        clf = LogisticRegression(random_state=0,**bayes_trials_results[0]['params'])
